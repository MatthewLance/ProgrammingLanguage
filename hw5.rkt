...
(define (racketlist->mupllist rks)
  (cond [(null? rks) (aunit)]
        [#t
         (apair (int (car rks)) (racketlist->mupllist (cdr rks)))]))

(define (mupllist->racketlist mps)
  (cond [(aunit? mps) null]
        [#t
         (cons (apair-e1 mps) (mupllist->racketlist (apair-e2 mps)))]))
