...
__global__ void maxpool(float *input, float *output, const int input_size, const int filter_size) {
    ...
    // out of bound
	if (col + blockDim.x > input_size || row + blockDim.y > input_size)
		return;

    // CHANGE
	int max_val = input[row*input_size + col];

	for (int r = 0; r < filter_size; r++) {
		for (int c = 0; c < filter_size; c++) {
			if (max_val < input[row*input_size + col + c])
				max_val = input[row*input_size + col + c];
		}
		row++;
	}
	output[blockDim.x + threadIdx.x + blockDim.y + threadIdx.y] = max_val;
}
